...
6. **Built-in cropper**: crop with preset or free ratios, 90° rotation, horizontal/vertical flip, free tilt (−90°…90°, with a triangle button that levels the tilt alone), a rule-of-thirds grid, and drag grips that work with both touch and mouse.
   ...
   **An edge carries twenty-four controls, twelve to each half, and each one is heard only in its own neighbourhood.** None of them appears on screen and none is ever chosen by hand — the three handles an edge shows are places to pull from, not controls. The pull is shared out among the controls within reach of the finger; a sixth of the edge either side, which with the controls' own reach comes to about a quarter of the edge each way. So one pull covers about half an edge, three of them cover the whole of it, and everything past a pull's reach is left exactly as it was — not nearly, exactly. Grabbed at its middle, the edge bows evenly about that point; grabbed at a quarter along, the far half does not move at all.

   The reach was a twelfth in v1.21.7 and is a sixth again in v1.21.8, because a twelfth was too fine to work with: every bump was small, and covering an edge took more strokes than settling it was worth. A sixth is the figure the three handles are spaced for — a quarter, a half, three quarters — and their spans meet without a gap and without the two outer ones touching each other at all.

   That locality is the whole of this. Until v1.21.7 an edge was one cubic with two controls, and both of them were heard along its entire length: a pull at a quarter along moved the middle by nine tenths of itself and the far end by a third. So correcting one half spoiled the other, correcting that spoiled the first, and settling a page went round in circles — an endless run of micro-corrections, each undoing the last. Now every correction is added to what is already there and takes nothing away: measured on the app's own drawing, shaping the right half of an edge moved the left half by 0.000 px. What one pull cannot do is make a *sharp* feature — the bump it lays down is a rounded one about a third of the edge wide — so shaping a whole edge takes three or four pulls rather than one, and each of them holds.
...
**The bend.** `Curve` adds a displacement inside the unit square, `qBend()`, applied **before** the perspective and never instead of it. That order is the whole design and must not be turned around. The obvious construction — filling a shape bounded by four curves directly, a Coons patch on the boundary — spreads the inside evenly and throws away the foreshortening the perspective has just worked out: the far edge of the page would be walked at the same rate as the near one. Applied first, the bend leaves the perspective to do its own job afterwards, and every control at rest gives the square back untouched, so a straight-edged crop is arithmetically the crop it was before any of this existed — the same file, byte for byte. Each edge is a uniform cubic B-spline over twenty-four controls (`BEND_N`), knotted so that the first and the last sit two knots inside the corners — which is what leaves every control silent at the corners themselves, so bending an edge never moves one. A control reaches two knots either side of itself and not a step further, and that locality is the point: the four edges are independent of one another and so, now, are the two halves of any one of them. `bendPull()` is the one place a bend is set. It shares a pull among the controls within `BEND_R` of the point pulled — a sixth of the edge either side — weighting each by how near it is and dividing by what those weights add up to at that point, which lands the curve exactly under the finger however many controls answered. Before v1.21.7 an edge was one cubic with two controls and the pull was shared between them cubed; both were heard everywhere, so a pull at a quarter along moved the middle by nine tenths of itself, and settling a lopsided page went round in circles. `bendPull()` takes only the half of the pull that runs **across** the edge, and that is not a detail. The other half slides the points of the curve about on a path whose shape has not changed, so nothing moves on screen — but it silently redefines which part of the page each column of the output is read from. Taking it was the app letting the finger say where the middle of an edge is, when the middle is the four corners' to say; measured against a simulated camera on a page ruled both ways and curled the way a book page curls, a line that should have run straight down came back bowed 2.2mm across a 210mm page, and a line that should have run straight across bowed 5.3mm where a flat crop of the same photo had it perfectly straight. Dropped, both read 0.0mm. `bAt()` gives the four controls a point can hear and how loudly, written into an array the caller keeps rather than returned, because the pixel loop asks millions of times; `bDt()` is the same four differentiated, for the fold test. `bendNear()` finds which edge the finger is on by walking the boundary, because a curve under a perspective has no tidy nearest point to solve for. `bendDrag()` serves all three ways of setting a bend, and its `side` argument is the whole difference between them: below zero for the middle handle and for a grab along the edge, which bend where they are, and 0 or 1 for one of the two that travel, which first take their place along the edge from the finger's own place along it. Which coordinate runs along an edge is simply which edge it is — the unit square's top and bottom run along u, its sides along v — and `spotClamp()` holds each to its own half. That along-edge reading is not a way back in for the half of a pull that is deliberately dropped: it moves the handle and never the bend, and the bend still only ever takes the half that runs across. `qBendOK()` samples the determinant of the map's Jacobian across the square and refuses a bend that would fold it, because a folded map has no straightening in it — the same objection a crossed-over quadrilateral gets, and a handle slides against that limit rather than sticking. The limit is far past anything a page asks for: folding does not begin until a pull of about a whole unit, and a page needs about a fifth. A row of local controls can put a fold in a smaller place than two controls ever could, so that square is walked more finely than it was, and so is every other walk along an edge — `qSizeBend()`, `qBendIn()` and `cropQuad()`'s bounding walk take 192 steps, the outline 48. Under-walking is not a cosmetic matter: a bulge whose tip the walk steps over is a bulge read from outside the photo, which comes back as the last row of pixels smeared across the page. `qSizeBend()` walks the four boundary curves rather than measuring across them, since a bent edge is longer than the line between its ends, and `cropQuad()` reads a region of the source that covers the bulge rather than just the corners.

**The corner.** `Custom` has a displacement of its own, `qKink()`, built the same way as the bend and applied at the same moment — before the perspective, every control at rest giving the square back untouched. It is the bend's opposite: where the bend puts a smooth cubic on each edge, this puts a tent on it, so the edge runs straight to a corner in its middle and straight on to the far end. One control to an edge, because one corner is one corner, and the displacement at the middle of an edge simply *is* that control, so the handle lands exactly where the finger is — including along the edge, which for a corner moves the corner and must be kept, and is the one place it differs from `bendPull()`. It is for a page that is *folded* rather than curled — two flat halves meeting on a hard line, which no curve has in it — and for anything whose outline is not a rectangle at all.

The two travel as one value and its length says which: eight numbers is corners, anything longer is a bend (a hundred and ninety-two, as the controls stand), nothing at all is the plain straight-edged crop. That is what lets `qSizeBend()`, `qAspectBend()`, `qBendIn()`, the editor's outline and the whole of `cropQuad()` serve both without any of them learning that there are two. `qKinkOK()` refuses a fold the way `qBendOK()` does, by the determinant of the Jacobian, sampled between the kink lines rather than across them since a tent has no derivative at its peak. Each shape keeps its own while the other is in front — a trip through `Curve` and back finds the corners where they were left — and only `Reset` clears either.
