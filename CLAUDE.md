...
6. **Built-in cropper**: crop with preset or free ratios, 90° rotation, horizontal/vertical flip, free tilt (−90°…90°, with a triangle button that levels the tilt alone), a rule-of-thirds grid, and drag grips that work with both touch and mouse.
   ...
   **Under `Normal`, the zoom is itself the crop.** Bringing a part of the photo up to fill the window is the shortest way there is to crop to it on a phone, so the rectangle is the whole of what the window shows: the whole photo at full size, exactly as before, and the part being looked at once zoomed in. It follows the window only while it still *is* the window — pull it in by a corner, or put a ratio on it, and it is yours from that moment and the zoom leaves it where it was put. A fresh rectangle is the window as well, so `Free` and the ratios lay themselves out inside what is being looked at. The four points of `Custom` and `Curve` are untouched by any of this, but the rectangle they are first placed on is the window, so choosing `Custom` while zoomed puts the four corners on what is being looked at rather than on the corners of a photo that is mostly off-screen.
   ...
   **The edge is the handle — in both shapes.** An edge is grabbed anywhere along it, never only at the dot sitting on it, because a dot is a small target on a page held at arm's length and the edge running past it is not. Under `Custom` the edge has nothing to bend, so the pull carries the whole edge — the two corners holding it — which is exactly what its midpoint handle does; under `Curve` the same pull bends it, as below. One walk serves a corner and the pair that carries an edge, so the handle and the edge itself cannot drift apart.
   ...
   **Each edge has two controls, not one, so it can take the lopsided shape a page has as it rolls off a spine rather than only an even bow. But neither the second control nor the choice between one and two appears anywhere: the edge itself is grabbed, at any point along it, and the pull is shared between that edge's two controls by how much each has to say where the finger landed — cubed, so the nearer one does most of the work. Grabbed at its middle, which is where the handle sits, they move together and the edge bows evenly, exactly as a page usually wants. Grabbed nearer an end, that end bends more. Two controls to an edge cannot make a local bump, so pulling one part of an edge does move the rest of it; settling a genuinely lopsided edge takes a pass or two, which is how adjusting by eye goes anyway.

   Measured before it was built: more controls per edge is not what limits this. Going from one to two cuts the residual bow of a book page by about a third, and going beyond two changes nothing at all — ten controls an edge, forty-four handles on screen, left the text bowing as much as one did. What remains is the interior: the bend fills the inside of the shape by blending its edges, and a page rolling off a spine does not blend that way. So two is where the useful ground ends, and that is why there is no third and no fourth. The bend is the crop's own memory the way the corners are — it survives a trip through `Custom` or `Normal`, and through `Apply`, and only `Reset` clears it. It costs almost nothing: a 12-megapixel photo measured 1393ms bent against 1365ms unbent, because the bend is two multiplications a pixel and the time goes on decoding and encoding either way.
...
**The bend.** `Curve` adds a displacement inside the unit square, `qBend()`, applied **before** the perspective and never instead of it. That order is the whole design and must not be turned around. The obvious construction — filling a shape bounded by four curves directly, a Coons patch on the boundary — spreads the inside evenly and throws away the foreshortening the perspective has just worked out: the far edge of the page would be walked at the same rate as the near one. Applied first, the bend leaves the perspective to do its own job afterwards, and every control at rest gives the square back untouched, so a straight-edged crop is arithmetically the crop it was before any of this existed — the same file, byte for byte. Each edge is the cubic through its two corners and its own two controls, so bending an edge never moves a corner, and the four edges are independent of one another. `bendPull()` is the one place a bend is set: it shares a pull between an edge's two controls by the cube of each one's say at the point pulled, which lands the curve exactly under the finger while keeping a pull at the middle even. `bendNear()` finds which edge the finger is on by walking the boundary, because a curve under a perspective has no tidy nearest point to solve for. `qBendOK()` samples the determinant of the map's Jacobian across the square and refuses a bend that would fold it, because a folded map has no straightening in it — the same objection a crossed-over quadrilateral gets, and a handle slides against that limit rather than sticking. The limit is far past anything a page asks for: folding does not begin until a pull of a whole unit, and a page needs about a fifth. `qSizeBend()` walks the four boundary curves rather than measuring across them, since a bent edge is longer than the line between its ends, and `cropQuad()` reads a region of the source that covers the bulge rather than just the corners.
...
The current version in `index.html` is **`v1.21.3`**.
