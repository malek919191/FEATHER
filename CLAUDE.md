...
6. **Built-in cropper**: crop with preset or free ratios, 90° rotation, horizontal/vertical flip, free tilt (−90°…90°, with a triangle button that levels the tilt alone), a rule-of-thirds grid, and drag grips that work with both touch and mouse.
   ...
   **An edge carries twenty-four controls, twelve to each half, and each one is heard only in its own neighbourhood.** None of them appears on screen and none is ever chosen by hand — there are still eight handles and no more. The edge itself is grabbed, anywhere along it, and the pull is shared out among the controls within reach of the finger; a twelfth of the edge either side, which with the controls' own reach comes to about a sixth of the edge each way. Everything past that is left exactly as it was — not nearly, exactly. Grabbed at its middle, which is where the handle sits, the edge bows evenly about that point; grabbed at a quarter along, the quarter beside it does not move, and neither does the middle.
...
The current version in `index.html` is **`v1.21.7`**.
