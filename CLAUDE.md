...
18. **Two modes**: a `Simple` / `Advanced` toggle in the top-right corner of the header, beneath the `Update` button and on exactly the same line as the language toggle opposite it. It is a fold, not a second app — one screen, one pipeline, the same settings underneath — and the choice is kept in `localStorage` under `fw-mode`. The app opens on `Simple`.
    ...
    The palette follows the mode. `Simple` is cream paper with dark olive ink; `Advanced` is dark maroon falling into black with cream ink; a gold band carrying the app's mark runs under the header in both. Both papers are drawn rather than photographed — the wash is a gradient and the tooth is a noise field the browser generates from an SVG filter written into the stylesheet, so there is still no image file and nothing fetched. Both layers are fixed to the screen, so they are rasterised once and a canvas the size of the whole document is never made. In `Advanced` a pressed button is filled with gold rather than a darker black, and a warning is gold rather than red: on the black half of the page a dark fill against a dark rest is a difference nobody can see, and red is invisible on maroon. `themePaint()` hands the palette's top colour to the root element and to the `theme-color` tag, neither of which can see a variable set on the body.
