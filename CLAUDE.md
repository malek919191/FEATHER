...
6. **Built-in cropper**: crop with preset or free ratios, 90° rotation, horizontal/vertical flip, free tilt (−90°…90°, with a triangle button that levels the tilt alone), a rule-of-thirds grid, and drag grips that work with both touch and mouse.
   ...
   **`Auto`, because the corners are the tedious part.** A fourth button beside `Rotate 90°`, `Flip H` and `Flip V`, and the only one in the cropper that does something rather than sets something. Pressed, it looks for the page — or the book, the whiteboard, the screen, whatever rectangular thing is in the shot — switches to `Custom` and puts the four corners on it. Pressed again it puts everything back exactly as it was: the shape, its bend, and which of the three was in front. Pulling any handle spends that undo, and so do `Reset`, `Normal` and moving to another photo; from any of those the shape is the user's own and there is nothing of the app's left to put back.

   It places the corners and nothing else. Whatever bend or hand-placed corner the shape was carrying is put back flat, so `Curve` is a fresh sheet to bend — which is the whole intent: let it find the corners, then curve the edges by hand. And the corners it places are ordinary four points like any other, so `Custom` and `Curve` hand the same shape back and forth exactly as they do for one placed by finger.

   It answers nothing rather than answer badly. A white page on a white table has no edges to find, and a shape whose edges are not really there is refused: the figure under the photo says so for a moment (`autoNone`) and whatever was already placed stays exactly where it was. It never touches a photo it was not pressed on, which is why it is a button and not something that happens on opening the editor — a batch of ten photos must not have its shapes moved by an app being helpful.

   **How it reads a photo.** All of it on the phone, no library, and nothing about it new to this app — the same kind of hand-rolled pass over pixels as `sharpen()` and `cropQuad()`. `autoEdges()` draws the photo into a copy about four hundred pixels across (`AUTO_LONG`), greys it, blurs it wide and measures its change from pixel to pixel with a Sobel pair. `autoThresh()` says what counts as an edge *in this photo*, by taking the strongest quarter of that change — a flat figure cannot, since a page on a dark table changes far harder than the same page on a light one. `autoLines()` is a Hough transform: every lit pixel votes for the straight line through it that runs across its own change, and only for the five directions its own gradient allows rather than for all hundred and eighty, which is both quicker and far less noisy. Its peaks are taken strongest-first, each held against the handful already kept rather than against its whole neighbourhood. `autoPick()` then pairs off lines that point roughly the same way and meet — if at all — far outside the picture, which is the test that tells two edges of a page from two lines that merely cross inside the frame: opposite edges of a page *are not parallel*, that being the entire reason this editor exists. Every pair is tried against every other, `autoCorners()` making the four crossings and refusing a shape that has folded over, `autoScore()` asking each of the four edges whether it is really there — the share of points along it that sit on strong change running the right way across it — with a lean towards the larger shape, since a page is usually the biggest flat thing in the frame. Below `AUTO_MIN` the answer is no.

   Three things in that are load-bearing and were each bought with a failure. **The blur is wide on purpose**, two passes of a box at a radius scaled to the working size, and it must not be narrowed back to something light: a page of text is row upon row of long, straight, strong edges, which is exactly what the vote hunts for, and at four hundred pixels across the lines of a paragraph outvoted the page they were printed on — the search came back with a shape somewhere inside the text block. Blurred this far a paragraph is flat grey, while the page's own edge, a step between two large plain areas, is merely softened, and a softened step still peaks in the same place. **The photo's own four sides are thrown in among the candidate lines**, worth a flat `AUTO_BORD` each, chosen so that three real edges carry a fourth that runs off the frame while four of them together never reach the mark — that is for the page photographed larger than the picture. **And what comes back is in the photo's own pixels**, ordered by where the four points fall in the view the user is actually looking at (`autoOrder()`), so a photo shown flipped hands its shape over exactly the way a hand-placed one does.

   The cost is about a fifth of a second on a phone, which is why the search is capped at forty line-pairs: every pair is tried against every other, so the cost goes up with the square of that, and sixty pairs was half a second on a busy photograph with no shape found any better. Measured against simulated photographs — square on, at an angle, in strong perspective, low contrast, noisy, dark on light, small in the frame, running off the edge — eighty of eighty were found with every corner within a few pixels, and nothing was claimed on any of fifty frames with no rectangle in them. In the browser, on a 1200×900 page shot at an angle, the worst corner landed five pixels out.
...
**Finding the four points.** `Auto` is a reader of the photo and never a writer of it: everything from `autoBlur()` to `autoPick()` works on a four-hundred-pixel grey copy that is thrown away, and all it ever produces is four numbers in the photo's own pixels, which `autoOrder()` then puts in the order the rectangle's own corners are in. Nothing downstream learns it exists — to `cropQuad()`, to `qAspect()`, to the pipeline and the PDF writer alike, four points found by the app are the same four points a finger would have placed. That is the whole of its contract, and it is what lets the feature be removed, or fail, without touching anything else.
...
The current version in `index.html` is **`v1.23`**.
